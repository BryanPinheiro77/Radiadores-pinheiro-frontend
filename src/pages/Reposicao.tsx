...
export default function Reposicao() {
  ...
  function toggleSelect(productId: number) {
    setOrderItems(prev =>
      prev.map(s =>
        s.productId === productId ? { ...s, selected: !s.selected } : s
      )
    )
  }
  ...
  function normalizeQuantity(productId: number) {
    setOrderItems(prev =>
      prev.map(s => {
        ...
        return {
          ...s,
          orderedQuantity: !s.orderedQuantity || parsed < 1 ? '1' : String(parsed)
        }
      })
    )
  }
  ...
  async function handleConfirm() {
    ...
    const hasInvalidQuantity = selectedItems.some(
      s => !s.orderedQuantity || Number(s.orderedQuantity) < 1
    )

    if (hasInvalidQuantity) {
      alert('Preencha a quantidade de todos os itens selecionados.')
      return
    }
              ...
              <table className="w-full text-sm">
                ...
                <tbody>
                  {orderItems.map(s => (
                    <tr
                      key={s.productId}
                      className={`border-b border-white/5 ${s.selected ? '' : 'opacity-40'}`}
                    >
