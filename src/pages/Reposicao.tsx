...
export default function Reposicao() {
  ...
  function addProductManually(product: Product) {
    ...
    setOrderItems(prev => [...prev, {
      ...
      orderedQuantity: Math.max(1, product.minStock - product.stock),
      selected: true
    }])
    setProductSearch('')
    setShowProductSearch(false)
  ...
  function updateQuantity(productId: number, quantity: number) {
    setOrderItems(prev => prev.map(s =>
      s.productId === productId ? { ...s, orderedQuantity: Math.max(1, quantity) } : s
    ))
  }
                  ...
                  {orderItems.map(s => (
                    <tr key={s.productId} className={`border-b border-white/5 ${s.selected ? '' : 'opacity-40'}`}>
                      ...
                      <td className="px-3 py-2 text-center">
                        <input
                          type="number" min="1" value={s.orderedQuantity}
                          onChange={e => updateQuantity(s.productId, parseInt(e.target.value) || 1)}
                          disabled={!s.selected}
