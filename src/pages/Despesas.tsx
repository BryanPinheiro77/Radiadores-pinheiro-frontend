...
export default function Despesas() {
  ...
  function fetchExpenses() {
    ...
    if (filterStart || filterEnd || filterCategory) {
      url = '/api/expenses/filter'
    }

    api.get<Page<Expense>>(`${url}?${params.toString()}`)
      .then(res => {
  ...
  async function handleSubmit(ev: React.FormEvent) {
    ...
    try {
      if (editing) {
        await api.put(`/api/expenses/${editing.id}`, body)
      } else {
  ...
  async function handleCategorySubmit(ev: React.FormEvent) {
    ...
    try {
      if (editingCategory) {
        await api.put(`/api/expense-categories/${editingCategory.id}`, categoryForm)
      } else {
        await api.post('/api/expense-categories', categoryForm)
      }
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <div className="bg-[#0d0f18] border border-white/10 rounded-xl p-4 flex items-center justify-between">
        <span className="text-white/40 text-sm">Total na página</span>
